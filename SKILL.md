...
> ⚠️ **本文件中的「定价」与「预言机地址」为占位/待确认值**，以文末「待你确认」区块为准。
> 最终数额由服务端单一环境变量驱动，客户端常量仅作离线提示。
> ⚠️ **可执行 `audit.jar` 暂未发布**：收费闭环待定，当前仓库不含可运行 jar，仅文档先行公开；闭环后推送。
...
|---|---|---|
| 预览报告 | **免费** | 本机离线跑，无限次 |
| 完整报告 | **¥0.2 / 次（占位）** | 按调用计费，一次一单 |

> ⚠️ 架构文档 §0 明确要求「Java 定价高于 Python 版（¥0.2）」。上表的 ¥0.2 仅是客户端离线提示占位，
> **最终价以你确认为准**（例如 ¥0.5 / ¥1 / ¥2 等）。真实金额以服务端环境变量 `AIPAY_AMOUNT`
> （Java 新容器）签发的 402 账单 + 回执校验为准，不受客户端常量影响。
...
1. **定价**：Java 版每解锁一次完整报告的价格？（文档 §0 要求高于 Python 版 ¥0.2；请给具体档位，
   我会同步改 `Paygate.PRICE_CNY` 占位与文档，并确认服务端新容器的 `AIPAY_AMOUNT`。）
2. **预言机 / 密钥**：新 Java 容器是（A）复用现有 `pay.seika.ltd/api/audit` 的同密钥对 + 新增一个
   `RESOURCE_ID` 商品（当前内嵌公钥已正确），还是（B）另起独立容器 + 新密钥对（我需更新内嵌 `SERVER_PUBKEY_PEM`）？
3. **x402 流程**：上方命令的 `resource-url` 是否就是最终 Java 预言机地址？`DEFAULT_ORACLE` 当前指向
   `https://pay.seika.ltd/api/audit`，若用新容器需改成新 endpoint。
